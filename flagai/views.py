...
@login_required
def index(request):
    manager_emails = [os.getenv("MANAGER_EMAIL_1"), os.getenv("MANAGER_EMAIL_2")]
    if request.user.email in manager_emails:
...
def update_report_status(request, report_id):
    ...
    try:
        ...
        subject = "FlagAI Report has new updates"
        ...
        send_notification(report, subject, message)
...
def anonymous_login(request):
    username = "anonymous"
    password = os.getenv("ANONYMOUS_PASSWORD")
    user = authenticate(request, username=username, password=password)
    if user is not None:
        auth_login(request, user)
        return redirect('/')
...
def create_report(request):
    if request.method == 'POST':
        ...
        subject = "FlagAI Report submitted successfully"
        message = "Your FlagAI report was submitted successfully. Your submission is now pending review."
        send_notification(report, subject, message)
...
def send_notification(report, subject, message):
    if report.user.username != "anonymous":
        send_mail(subject, message, os.getenv('EMAIL_HOST_USER'), [report.user.email], auth_user=os.getenv('EMAIL_HOST_USER'), auth_password=os.getenv('EMAIL_HOST_PASSWORD'))
